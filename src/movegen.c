...
static void generate_pawn_moves(const position_t *position,
    ...
    while (pawns) {
        ...
        int targets[2] = {from + step - 1, from + step + 1};
        for (int i = 0; i < 2; ++i) {
            if ((i == 0 && !file) || (i == 1 && file == 7)) continue;
            to = targets[i];
            if ((unsigned)to >= 64u) continue;
            int flags = 0;
            if (to == position->en_passant) {
                flags = MOVE_CAPTURE | MOVE_EN_PASSANT;
            } else if (position->board[to] != NO_PIECE &&
                       piece_color(position->board[to]) != side) {
                flags = MOVE_CAPTURE;
            } else {
                continue;
            }
...
static void generate_leaper_moves(const position_t *position,
    ...
    while (pieces) {
        ...
        bitboard_t attacks = attack_table[from] & ~own;
        if (captures_only) attacks &= enemy;
        while (attacks) {
...
static void generate_slider_moves(const position_t *position,
    ...
    while (pieces) {
        ...
        bitboard_t attacks;
        if (type == BISHOP) {
            attacks = generate_bishop_attacks(from, position->occupancy[ALL_PIECES]);
        } else if (type == ROOK) {
            attacks = generate_rook_attacks(from, position->occupancy[ALL_PIECES]);
        } else {
            attacks = generate_bishop_attacks(from, position->occupancy[ALL_PIECES]) |
                      generate_rook_attacks(from, position->occupancy[ALL_PIECES]);
        }
        attacks &= ~own;
        if (captures_only) attacks &= enemy;
        while (attacks) {
...
static void generate_castling_moves(const position_t *position, move_list_t *list) {
    int side = position->side_to_move;
    int opponent = side ^ 1;
    if (side_in_check(position, side)) return;
    ...
    if (side == WHITE && position->board[4] == WHITE_KING) {
        if ((position->castling & CASTLE_WHITE_KING) &&
            ...
            !square_is_attacked(position, 5, opponent) &&
            !square_is_attacked(position, 6, opponent)) {
        ...
        if ((position->castling & CASTLE_WHITE_QUEEN) &&
            ...
            !square_is_attacked(position, 3, opponent) &&
            !square_is_attacked(position, 2, opponent)) {
        ...
        if ((position->castling & CASTLE_BLACK_KING) &&
            ...
            !square_is_attacked(position, 61, opponent) &&
            !square_is_attacked(position, 62, opponent)) {
        ...
        if ((position->castling & CASTLE_BLACK_QUEEN) &&
            ...
            !square_is_attacked(position, 59, opponent) &&
            !square_is_attacked(position, 58, opponent)) {
...
void generate_moves(const position_t *position, move_list_t *list, bool captures_only) {
    ...
    generate_pawn_moves(position, list, captures_only);
    generate_leaper_moves(position, list, captures_only,
                          piece_offset + KNIGHT, knight_attacks);
    generate_slider_moves(position, list, captures_only,
    ...
    if (!captures_only) generate_castling_moves(position, list);
}
