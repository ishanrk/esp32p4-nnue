...
The selected 4x128 baseline is inspired by HalfKP but does not use an exact
king-square index.
Instead, each perspective conditions its nonking piece-square features on one
of four mirrored king buckets.
...
The normalized king file directly selects zero through three. This produces
four buckets with one full-rank band. Reproducible 8-bucket and 16-bucket
experiment profiles use the same files with two four-rank bands or four
two-rank bands. The profile comparison page defines those regions and records
...
pawn, knight, bishop, rook, queen. A perspective has 4 times 640 or 2,560
...
The selected hidden width is 128 and activation remains clipped ReLU. The
