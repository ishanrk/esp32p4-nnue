...
export function IntegrationGuide() {
  return <Page title="Connect your own engine">
    ...
    <p>Your engine can use its own evaluator and board identity. Start with the working example below, then replace its engine and serial functions with yours. The same website handles the connection.</p>
    <Section title="1. Run the complete example">
      ...
      <p>The host example uses this project’s classical chess engine, which scores material and piece placement. It reports its own identity with no neural model. Run it on your computer to check the adapter before testing your board’s USB connection.</p>
    ...
    <Section title="2. Implement the engine callbacks">
      <p>Implement <code>get_info</code>, <code>get_capabilities</code>, <code>set_position</code> and <code>search</code> in <code>board_protocol_backend_t</code>. The browser sends protocol messages over serial. The firmware dispatcher decodes them and calls these local C functions.</p>
      ...
      <p>Map your FEN parser to this pattern. The borrowed string pointer is valid only during the callback. A fresh FEN resets search history and supplies the halfmove counter. Full game repetition detection also needs the earlier sequence of positions.</p>
      ...
      <p>Return <code>BOARD_ERROR_NONE</code> on success, <code>BOARD_ERROR_POSITION_REQUIRED</code> if no position was accepted, or <code>BOARD_ERROR_ENGINE_UNAVAILABLE</code> if search cannot run, including an allocation failure. Fill the borrowed result before returning and leave the accepted root position usable.</p>
    ...
    <Section title="Protocol messages and callbacks">
      ...
      <p>The checksum covers version through payload. The shared implementation uses CRC32 ISO HDLC with reflected polynomial <code>0xedb88320</code>, initial value and final XOR <code>0xffffffff</code>. ASCII <code>123456789</code> checks to <code>0xcbf43926</code>. CRC detects accidental corruption. Device identity remains an unauthenticated firmware report.</p>
